...
# what should happen if a file is owned by root? what if a cgi script?
...
class SecureWebServer:
	...
	def __init__(self, listenerUid, listenerGid, port=80, host='', unixSocketPath="/tmp/sws.peerweb.it"):
		...
		rootProcess = Process (target=self.rootProcessHandler,args=(self.listener,))
		rootProcess.start()
		...
		while 1:
			...
			clientHandlerProcess = Process (target=self.clientProcessHandler,args=(conn,addr,self.listener,))
...
try:
	s = SecureWebServer(1000,1000,port)
